	...
	setmetatable(_L,{
		...
		__newindex=function(_,k,v)
			if lv[k] then
				...
				_bp_log_("error:invalid local name:",k)
			end
		end,
	...
	setmetatable(_U,{
		...
		__newindex=function(_,k,v)
			if uv[k] then
				...
				_bp_log_("error:invalid upvalue name",k)
			end
		end,
		__tostring=function(_)
...
local function bp_list_name(name)
	...
	_bp_log_("["..name.."]",_bp_list[name] and "on" or "off")
	if t=="table" then
		for _,v in ipairs(_bp_desc[name]) do
			_bp_log_("----",v)
		end
	elseif t=="string" then
		_bp_log_("---",_bp_desc[name])
	end
end
...
local function bp_show(n)
	if type(n)=="number" then
		...
		_bp_log_("break point:",n,
			_bp_unnamed[func] and "on" or "off",
			_bp_unnamed_desc[func]
			)	
	else
		_bp_log_("break point:",n,_bp_named[n] and "on" or "off")
